function [StegoImage]= My_Darker_Embedding_New(OriginalImage,message,Length_of_Message,darker)
...
capacity_of_image = (darker)

number_of_charachters = darker/8 
...
for i = 1 : height
    ...
    for j = 1 : width
        ...
        if (counter <= Length_of_Message_To_Be_Embedded)
            ...
            elseif ((R_MSB==0) && (G_MSB==0) && (B_MSB==0))
                
                fprintf('\n\npixel no: %d,%d\n',i,j);
                
                Message_bit = num2str(Msg_To_Be_Embedded(counter));
                
                B_LSB = num2str(bitget(B,1));
                ...
                fprintf('Blue LSB: %d\n',LSB)
                disp(['msgbit is: ',Message_bit]);
                fprintf('<strong>Embedd Message bit in LSB.. </strong>\n');
                
                %%
                
                
                if (Message_bit ~= B_LSB)
